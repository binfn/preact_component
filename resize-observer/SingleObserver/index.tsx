...
import { CollectionContext } from '../Collection.tsx';

export interface SingleObserverProps extends ResizeObserverProps {
  children: React.ComponentChildren | ((ref: React.RefObject<Element>) => React.ComponentChildren);
}
...
export default function SingleObserver(props: SingleObserverProps) {
  ...
  const onCollectionResize = React.useContext<any>(CollectionContext);
  ...
  React.useEffect(() => {
    const currentElement: HTMLElement =
      findDOMNode(elementRef.current! as HTMLElement ) || findDOMNode(wrapperRef.current! as React.Component);
