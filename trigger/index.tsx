...
import contains from '../util/Dom/contains.ts';
...
import addEventListener from '../util/Dom/addEventListener.ts';
import Portal from '../util/Portal.tsx';

import {
  getAlignFromPlacement,
  getAlignPopupClassName,
} from './utils/alignUtil.ts';
import Popup from './Popup/index.tsx';
import type { PopupInnerRef } from './Popup/PopupInner.tsx';
import TriggerContext from './context.ts';
import type {
  ActionType,
  AlignType,
  TransitionNameType,
...
import { BuildInPlacements } from './interface.ts';
...
export interface TriggerProps {
  ...
  popup: React.ComponentChild | (() => React.ComponentChild);
  popupStyle?: React.JSX.CSSProperties;
  prefixCls?: string;
   ...
   */
  getTriggerDOMNode?: (node: React.Component<any> | Element
    //React.ReactInstance
    ) => HTMLElement;
...
export function generateTrigger(
  ...
  class Trigger extends React.Component<TriggerProps, TriggerState> {
    ...
    triggerRef = React.createRef<React.Component<any> | HTMLElement>();
    ...
    getRootDomNode = (): HTMLElement => {
      ...
      return React.Compat.findDOMNode(this) as HTMLElement;
    };

    getPopupClassNameFromAlign = (align:any) => {
      const className = [];
      const {
        popupPlacement,
        builtinPlacements,
    ...
    getComponent = () => {
      ...
      const mouseProps: React.JSX.HTMLAttributes<HTMLElement> = {};
    ...
    delaySetPopupVisible(visible: boolean, delayS: number, event?: MouseEvent) {
      ...
      if (delay) {
        ...
        this.delayTimer = window.setTimeout(() => {
          this.setPopupVisible(visible, point?point:undefined);
          this.clearDelayTimer();
        }, delay);
      } else {
    ...
    clearOutsideHandler() {
      ...
      if (this.contextMenuOutsideHandler1) {
        ...
        this.contextMenuOutsideHandler1 = undefined;
      }

      if (this.contextMenuOutsideHandler2) {
        this.contextMenuOutsideHandler2.remove();
        this.contextMenuOutsideHandler2 = undefined;
      }

      if (this.touchOutsideHandler) {
        this.touchOutsideHandler.remove();
        this.touchOutsideHandler = undefined;
      }
    }
    ...
    fireEvents(type: string, e: Event) {
      // @ts-ignore original
      const childCallback = (this.props.children as React.VNode).props[
        type
      ];
      if (childCallback) {
        childCallback(e);
    ...
    render() {
      ...
      const { children, forceRender, alignPoint, className, autoDestroy } =
        this.props;
      const child = React.Compat.Children.only(children) as React.VNode;
      const newChildProps: React.JSX.HTMLAttributes<HTMLElement> & { key: string } = {
        key: 'trigger',
      };
      ...
      // =================================== Render ===================================
      const childrenClassName = `${(child as any)?.props?.className??''} ${className?className:''}`
      // classNames(
      ...
      let portal: React.ComponentChild;
