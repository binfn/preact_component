...
import KeyCode from '../util/KeyCode.ts';
...
import {
  ...
  getTreeNodeProps,
  fillFieldNames,
} from './utils/treeUtil.ts';
import NodeList, { MOTION_KEY, MotionEntity, NodeListRef } from './NodeList.tsx';
import TreeNode from './TreeNode.tsx';
import { conductCheck } from './utils/conductUtil.ts';
...
export interface TreeProps<TreeDataType extends BasicDataNode = DataNode> {
  ...
  titleRender?: (node: TreeDataType) => React.VNode;
  ...
  onFocus?: React.JSX.FocusEventHandler<HTMLDivElement>;
  onBlur?: React.JSX.FocusEventHandler<HTMLDivElement>;
  onKeyDown?: React.JSX.KeyboardEventHandler<HTMLDivElement>;
  onContextMenu?: React.JSX.MouseEventHandler<HTMLDivElement>;
  ...
  onRightClick?: (info: { event: MouseEvent; node: EventDataNode }) => void;
  ...
  rootStyle?: React.JSX.CSSProperties;
}
...
> {
  ...
  dragStartMousePosition :any= null;

  dragNode?: NodeInstance<TreeDataType>;

  currentMouseOverDroppableNodeKey:any = null;
  ...
  static getDerivedStateFromProps(props: TreeProps, prevState: TreeState) {
    ...
    // ================== Tree Node ==================
    let treeData: DataNode[]|undefined=undefined;
   ...
   */
  onNodeDragEnter = (event: React.JSX.TargetedDragEvent<HTMLDivElement>, node: NodeInstance<TreeDataType>) => {
  ...
  onNodeDragOver = (event: React.JSX.TargetedDragEvent<HTMLDivElement>, node: NodeInstance<TreeDataType>) => {
  ...
  // @ts-ignore original
  onNodeDrop = (event: React.JSX.TargetedDragEvent<HTMLDivElement>, node, outsideTree: boolean = false) => {
  ...
  onFocus: React.JSX.FocusEventHandler<HTMLDivElement> = (...args) => {
  ...
  onBlur: React.JSX.FocusEventHandler<HTMLDivElement> = (...args) => {
  ...
  onNodeExpand = (e: React.JSX.TargetedMouseEvent<HTMLDivElement>, treeNode: EventDataNode) => {
  ...
  onKeyDown: React.JSX.KeyboardEventHandler<HTMLDivElement> = event => {
      ...
      switch (event.which) {
        ...
        case KeyCode.LEFT: {
          ...
          if (expandable && expandedKeys.includes(activeKey!)) {
            this.onNodeExpand({} as React.JSX.TargetedMouseEvent<HTMLDivElement>, eventNode);
        ...
        case KeyCode.SPACE: {
          if (
            ...
            this.onNodeCheck(
              {} as React.JSX.TargetedMouseEvent<HTMLDivElement>,
              eventNode,
            ...
            this.onNodeSelect({} as React.JSX.TargetedMouseEvent<HTMLDivElement>, eventNode);
          }
          break;
        }
      }
  ...
  render() {
    ...
    const domProps: React.JSX.HTMLAttributes<HTMLDivElement> = pickAttrs(this.props, {
      aria: true,
      data: true,
    });

    // It's better move to hooks but we just simply keep here
    let draggableConfig: DraggableConfig|undefined=undefined;
    if (draggable) {
